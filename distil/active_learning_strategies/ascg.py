...
class ASCG(Strategy):
    """
    This strategy implements a new Submodular Conditional Gain (SCG) selection paradigm for active learning. In this selection 
    paradigm, points from the unlabeled dataset are chosen in such a way that the submodular conditional gain 
    between this set of points and the current labeled set is maximized. Doing so allows a practitioner to select 
    ...
    def select(self, budget):
        ...
        if(embedding_type == "gradients"):
            unlabeled_data_embeddings = self.get_grad_embedding(self.unlabeled_dataset, True, gradType)
            labeled_data_embeddings = self.get_grad_embedding(self.labeled_dataset, False, gradType)
        elif(embedding_type == "features"):
            unlabeled_data_embeddings = self.get_feature_embedding(self.unlabeled_dataset, True, layer_name)
            labeled_data_embeddings = self.get_feature_embedding(self.labeled_dataset, False, layer_name)
        else:
            raise ValueError("Provided representation must be one of gradients or features")
        
        #Compute image-image kernel
        data_sijs = (submodlib.helper.create_kernel(X=unlabeled_data_embeddings.cpu().numpy(), metric=metric, method="sklearn") + 1.) / 2.
        #Compute private-private kernel
        if(self.args['scg_function']=='logdetcg'):
            private_private_sijs = (submodlib.helper.create_kernel(X=labeled_data_embeddings.cpu().numpy(), metric=metric, method="sklearn") + 1.) / 2.
        #Compute image-private kernel
        private_sijs = (submodlib.helper.create_kernel(X=labeled_data_embeddings.cpu().numpy(), X_rep=unlabeled_data_embeddings.cpu().numpy(), metric=metric, method="sklearn") + 1.) / 2.
        
        if(self.args['scg_function']=='flcg'):
            obj = submodlib.FacilityLocationConditionalGainFunction(n=unlabeled_data_embeddings.shape[0],
                                                                      num_privates=labeled_data_embeddings.shape[0],  
                                                                      data_sijs=data_sijs, 
        ...
        if(self.args['scg_function']=='gccg'):
            ...
            obj = submodlib.GraphCutConditionalGainFunction(n=unlabeled_data_embeddings.shape[0],
                                                                      num_privates=labeled_data_embeddings.shape[0],
                                                                      lambdaVal=lambdaVal,  
                                                                      data_sijs=data_sijs, 
                                                                      private_sijs=private_sijs, 
        ...
        if(self.args['scg_function']=='logdetcg'):
            lambdaVal = self.args['lambdaVal'] if 'lambdaVal' in self.args else 1
            obj = submodlib.LogDeterminantConditionalGainFunction(n=unlabeled_data_embeddings.shape[0],
                                                                      num_privates=labeled_data_embeddings.shape[0],
                                                                      lambdaVal=lambdaVal,  
                                                                      data_sijs=data_sijs, 
                                                                      private_sijs=private_sijs,
